...
const FOLDER_PATH = process.envFOLDER_PATH || '/tmp/files_manager';
...
class FilesController {
	...
	static async getShow(request, response) {
		const fileId = request.paarams.id;
	...
	static async getIndex(request, response) {
		const { userId } = await userUtils.getUser(request);
	...
	static async putPublish(request, response) {
		const { error, code, updateFile } = await fileUtils.publishUnpublish(
			request,
			true,
		);
	...
	static async putUnpublish(request, response) {
		...
		if (error) return response.status(code).send(updatedFile);
	...
	static async getFile(request, response) {
		...
		if (!file || !fileUtils.isOwnerAndPubblic(file, userId)) { return response.status(404).send({ error: 'Not found' }); }

		if (file.type === 'folder') {
			return response
			.status(400)
			.send({ error: "A folder doesnt have content" });
		}
